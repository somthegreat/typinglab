...
DROP FUNCTION IF EXISTS public.get_or_create_daily_challenge(date);

CREATE FUNCTION public.get_or_create_daily_challenge(p_date date DEFAULT CURRENT_DATE)
...
BEGIN
  ...
  WHERE dc.challenge_date = get_or_create_daily_challenge.p_date
  LIMIT 1;
  ...
  IF get_or_create_daily_challenge.p_date < CURRENT_DATE THEN
    RAISE EXCEPTION 'Cannot create challenges for past dates';
  END IF;
  
  IF get_or_create_daily_challenge.p_date > CURRENT_DATE + INTERVAL '7 days' THEN
  ...
  v_index := (EXTRACT(DOY FROM get_or_create_daily_challenge.p_date)::integer % array_length(v_texts, 1)) + 1;
  ...
  VALUES (get_or_create_daily_challenge.p_date, v_text_content, v_target_wpm, v_target_accuracy, v_reward_points, 'standard')
