...
CREATE FUNCTION public.get_or_create_daily_challenge(p_date date DEFAULT CURRENT_DATE)
 RETURNS TABLE(out_id uuid, out_challenge_date date, out_text_content text, out_target_wpm integer, out_target_accuracy numeric, out_reward_points integer, out_challenge_type text)
 LANGUAGE plpgsql
 SECURITY DEFINER
